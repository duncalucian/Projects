...
public class Game extends Canvas implements Runnable {
    ...
    private static BufferedImage level[] = new BufferedImage[5];
    private static BufferedImage coinSprite = null;
    private static BufferedImage flagSprite = null;
    private static BufferedImage backgorund = null;
    ...
    private void init() {
        ...
        try {
            ...
            level[1] = ImageIO.read(getClass().getResource("/level1.png"));
            level[2] = ImageIO.read(getClass().getResource("/level2.png"));
            ...
            backgorund = ImageIO.read(getClass().getResource("/back1.png"));
            flagSprite = ImageIO.read(getClass().getResource("/flag.png"));
    ...
    private void renderer() {
        ...
        g.drawImage(backgorund, 0, 0, WIDTH, HEIGHT, null);
    ...
    private static void drawLevel(){
        int i = 0, j = 0;
        for (i = 0; i < level[levelNumber].getHeight(); ++i)
            for (j = 0; j < level[levelNumber].getWidth(); ++j) {
                int rgb = level[levelNumber].getRGB(j, i);
                ...
                if (red == 0 && green == 255 && blue == 255)
                    handler.addTile(new Flag(j *  64 - 36, i * 64 - 136, 100, 200, false, Id.flag, handler));
    ...
    public static Rectangle getVisibleArea(){
        for(Entity en: handler.entity){
            if(en.getId() == Id.player) return new Rectangle(en.getX() - WIDTH / 2 -5, en.getY() - HEIGHT /2 -5, WIDTH + 10, HEIGHT +10);
        }
        return null;
    }
    ...
    public static BufferedImage getFlagSprite() {
        return flagSprite;
    }
